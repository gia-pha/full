  ...
  editPerson(p) {
    ...
    return `
      <div class="space-y-3 sm:space-y-4">
        ${this.input(this.t.member.name, 'modal-person-name', p.data.firstName + ' ' + p.data.lastName)}
        <div class="grid grid-cols-2 gap-3">${this.input(this.t.member.birthYear, 'modal-person-birth', p.data.birthYear || '', 'number')}${this.input(this.t.member.deathYear, 'modal-person-death', p.data.deathYear || '', 'number')}</div>
    ...
    return `
      <div class="space-y-4">
        <div class="p-3 sm:p-4 bg-gray-50 rounded-lg">
          ...
          <p class="font-medium text-gray-800 text-sm">${person.data.firstName + ' ' + person.data.lastName}</p>
  ...
  bindEvents() {
    ...
    this.container.querySelector('.modal-save-person')?.addEventListener('click', () => {
      ...
      if (p) {
        ...
        if (n) { const parts = n.value.trim().split(' '); p.data.lastName = parts.pop() || ''; p.data.firstName = parts.join(' ') || ''; }
        if (b) p.data.birthYear = parseInt(b.value) || null;
        if (d) p.data.deathYear = parseInt(d.value) || null;
