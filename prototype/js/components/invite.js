    ...
    this.container.innerHTML = `
      <div class="h-full overflow-y-auto bg-white">
        ...
        <h3 class="font-bold text-emerald-800 text-lg mb-2">${this.t.invite.qrTitle}</h3>
         ...
         <button class="invite-copy px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-medium transition-colors">📋 ${this.t.invite.copyLink}</button>
               <button class="invite-share px-5 py-2.5 bg-white border border-emerald-300 hover:bg-emerald-50 text-emerald-600 rounded-xl text-sm font-medium transition-colors">📤 ${this.t.invite.share}</button>
            </div>
          </div>
          <div class="max-w-2xl mx-auto">
            ...
            ${invites.length === 0 ? `<div class="text-center py-12 text-gray-400"><p class="text-base">${this.t.invite.noInvites}</p></div>` : `<div class="space-y-3">${invites.map(inv => `
              <div class="flex items-center justify-between p-5 bg-gray-50 rounded-xl">
                ...
                <span class="px-4 py-1.5 rounded-full text-sm font-medium bg-emerald-100 text-emerald-700">${this.t.invite.joinedStatus}</span>
              </div>
            `).join('')}</div>`}
          </div>
        </div>
