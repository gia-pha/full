...
class GpMemberCard extends GpComponent {
  render() {
    ...
    if (!person) { this.innerHTML = ''; return; }
    ...
    requestAnimationFrame(() => {
      const sheet = this.querySelector('.member-sheet');
  ...
  bindEvents() {
    this.querySelectorAll('.member-close').forEach(b => b.onclick = () => this.store.setSelectedPerson(null));
    this.querySelector('.member-overlay')?.addEventListener('click', () => this.store.setSelectedPerson(null));
    this.querySelectorAll('.member-link').forEach(l => l.onclick = () => this.store.setSelectedPerson(l.dataset.id));
    this.querySelector('.member-edit')?.addEventListener('click', () => this.store.openModal('edit-person', this.store.getSelectedPerson()));
    this.querySelector('.member-delete')?.addEventListener('click', () => { if (confirm('Xoa thanh vien nay?')) this.store.setSelectedPerson(null); });
  }
}

customElements.define('gp-member-card', GpMemberCard);
export { GpMemberCard };
