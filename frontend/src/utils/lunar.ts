...
// For each year: [month1Days, month2Days, ..., leapMonthNumber, leapMonthDays]
// leapMonthNumber = 0 means no leap month
...
export function solarToLunar(
  ...
  for (const [yearStr, lny] of Object.entries(lunarNewYears)) {
    ...
    if (targetDate >= lnyDate && targetDate < nextLnyDate) {
      ...
      if (!data) {
        return { year, month: 1, day: 1, isLeapMonth: false };
      }
      ...
      for (let i = 0; i < data.months.length; i++) {
        ...
        if (data.leapMonth !== 0 && i === data.leapMonth) {
          isLeapMonth = true;
        }
        if (dayRemaining < monthDays) {
          const month = data.leapMonth > 0 && i >= data.leapMonth ? i : i + 1;
          return { year, month, day: dayRemaining + 1, isLeapMonth };
        }
