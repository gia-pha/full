...
export function getGenderIconSvg(gender: string): string {
  const path = gender === 'F' ? GENDER_ICON_PATHS.F : GENDER_ICON_PATHS.M;
  return `<svg viewBox="0 0 24 24" fill="currentColor"><path d="${path}"/></svg>`;
}

export function getGenderIcon(gender: string): SVGTemplateResult {
  const path = gender === 'F' ? GENDER_ICON_PATHS.F : GENDER_ICON_PATHS.M;
