...
class EventsComponent {
      ...
      <div class="h-full overflow-y-auto bg-heritage-cream">
        <div class="p-4 sm:p-6 lg:p-8 border-b border-gray-200 flex items-center justify-between bg-white">
          <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl sm:text-2xl font-bold font-serif text-gray-800">${this.t.events.title}</h2>
          </div>
          ${canEdit ? `<button class="events-add px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-lg text-sm font-medium transition-all shadow-md shadow-purple-200">+ ${this.t.events.addEvent}</button>` : ''}
        </div>
        <div class="p-4 sm:p-6 lg:p-8">
          <div class="flex gap-3 mb-6">
            <button class="events-tab px-5 py-2.5 rounded-xl text-sm font-medium transition-all ${this.tab === 'upcoming' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md shadow-purple-200' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200'}" data-tab="upcoming">📅 ${this.t.events.upcoming} (${upcoming.length})</button>
            <button class="events-tab px-5 py-2.5 rounded-xl text-sm font-medium transition-all ${this.tab === 'past' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md shadow-purple-200' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200'}" data-tab="past">📋 ${this.t.events.past} (${past.length})</button>
          </div>
  ...
  renderCard(event, canEdit) {
    ...
    return `
      <div class="heritage-card overflow-hidden hover:shadow-lg transition-shadow">
          ...
          <div class="flex items-start justify-between mb-3">
            <div class="flex-1 min-w-0">
              <div class="flex flex-wrap items-center gap-2 mb-2">
                ...
                <span class="px-3 py-1 text-xs rounded-full font-medium ${event.status === 'upcoming' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-500'}">${event.status === 'upcoming' ? this.t.events.upcomingLabel : this.t.events.pastLabel}</span>
              </div>
              <h3 class="font-bold font-serif text-gray-800 text-lg">${event.title}</h3>
            </div>
          ...
          ${event.images?.length > 0 ? `
            <div class="mb-4">
              ...
              <div class="flex gap-3 overflow-x-auto pb-2">
                ${event.images.map(img => `<div class="w-36 h-24 lg:w-48 lg:h-32 bg-gray-100 rounded-xl overflow-hidden flex-shrink-0 shadow-sm"><img src="${img}" class="w-full h-full object-cover" loading="lazy" /></div>`).join('')}
              </div>
            </div>
          ` : ''}
