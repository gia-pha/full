class ModalComponent {
  ...
  render() {
    ...
    this.container.innerHTML = `
      <div class="modal-overlay fixed inset-0 z-50 bg-black/40 flex items-end sm:items-center justify-center">
        <div class="modal-content bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl shadow-2xl max-h-[90vh] sm:m-4 overflow-hidden flex flex-col border border-purple-100">
          <div class="sticky top-0 bg-white px-4 sm:px-6 py-3 sm:py-4 border-b flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <h3 class="font-semibold font-serif text-gray-800 text-sm sm:text-lg">${titles[type] || 'Modal'}</h3>
            </div>
  ...
  input(label, cls, value = '', type = 'text', extra = '') {
    return `<div><label class="text-xs sm:text-sm text-gray-600 font-medium">${label}</label><input type="${type}" class="${cls} w-full mt-1 px-3 sm:px-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 bg-amber-50/30" value="${value}" ${extra} /></div>`;
  }

  textarea(label, cls, value = '', rows = 3) {
    return `<div><label class="text-xs sm:text-sm text-gray-600 font-medium">${label}</label><textarea class="${cls} w-full mt-1 px-3 sm:px-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 bg-amber-50/30" rows="${rows}">${value}</textarea></div>`;
  }
    ...
    return `
      <div class="space-y-3 sm:space-y-4">
        ...
        <div class="flex gap-2 pt-2">
          <button class="modal-save-person flex-1 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 active:from-purple-700 active:to-indigo-700 text-white rounded-lg text-sm font-medium shadow-md shadow-purple-200">${this.t.profile.save}</button>
  ...
  editClan(c) {
    ...
    return `
      <div class="space-y-3 sm:space-y-4">
        ...
        <div class="flex gap-2 pt-2">
          <button class="modal-save-clan flex-1 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 active:from-purple-700 active:to-indigo-700 text-white rounded-lg text-sm font-medium shadow-md shadow-purple-200">${this.t.profile.save}</button>
          <button class="modal-cancel px-4 py-2.5 bg-gray-100 active:bg-gray-200 text-gray-600 rounded-lg text-sm font-medium">${this.t.profile.cancel}</button>
        </div>
      </div>
    `;
    ...
    return `
      <div class="space-y-3 sm:space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div><label class="text-xs sm:text-sm text-gray-600 font-medium">Loai</label><select class="modal-event-type w-full mt-1 px-3 sm:px-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 bg-amber-50/30"><option value="memorial" ${event?.type === 'memorial' ? 'selected' : ''}>Giô to</option><option value="meeting" ${event?.type === 'meeting' ? 'selected' : ''}>Hoi nghi</option><option value="reunion" ${event?.type === 'reunion' ? 'selected' : ''}>Quen ma</option><option value="anniversary" ${event?.type === 'anniversary' ? 'selected' : ''}>Ky niem</option></select></div>
        </div>
        ${this.input(this.t.events.location, 'modal-event-location', event?.location || '')}
        ${this.input('Map URL', 'modal-event-map', event?.mapUrl || '', 'url')}
        <div class="flex gap-2 pt-2">
          <button class="modal-save-event flex-1 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 active:from-purple-700 active:to-indigo-700 text-white rounded-lg text-sm font-medium shadow-md shadow-purple-200">${this.t.profile.save}</button>
          <button class="modal-cancel px-4 py-2.5 bg-gray-100 active:bg-gray-200 text-gray-600 rounded-lg text-sm font-medium">${this.t.profile.cancel}</button>
        </div>
      </div>
    `;
  ...
  fundTxn(data) {
    ...
    return `
      <div class="space-y-3 sm:space-y-4">
        ...
        <div><label class="text-xs sm:text-sm text-gray-600 font-medium">${this.t.fund.person}</label><select class="modal-txn-person w-full mt-1 px-3 sm:px-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 bg-amber-50/30">${members.map(m => `<option value="${m.id}">${m.data['first name'] + ' ' + m.data['last name']}</option>`).join('')}</select></div>
        <div><label class="text-xs sm:text-sm text-gray-600 font-medium">${this.t.fund.event} (optional)</label><select class="modal-txn-event w-full mt-1 px-3 sm:px-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 bg-amber-50/30"><option value="">-- Khong co --</option>${events.map(e => `<option value="${e.id}">${e.title}</option>`).join('')}</select></div>
        <div class="flex gap-2 pt-2">
          <button class="modal-save-txn flex-1 py-2.5 ${data.type === 'contribution' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 active:from-purple-700 active:to-indigo-700' : 'bg-red-600 active:bg-red-700'} text-white rounded-lg text-sm font-medium shadow-md">${data.type === 'contribution' ? 'Dong gap' : 'Chi tieu'}</button>
          <button class="modal-cancel px-4 py-2.5 bg-gray-100 active:bg-gray-200 text-gray-600 rounded-lg text-sm font-medium">${this.t.profile.cancel}</button>
        </div>
      </div>
    `;
  ...
  changeRole(data) {
    ...
    return `
      <div class="space-y-4">
        <div class="p-3 sm:p-4 bg-purple-50 rounded-lg border border-purple-100">
          ...
          <p class="text-xs text-gray-500 mt-1">${this.t.member.role}: <span class="text-purple-600">${this.t.admin.roles[data.currentRole]}</span></p>
        </div>
        <div><label class="text-xs sm:text-sm text-gray-600 font-medium">Vai tro moi</label><select class="modal-new-role w-full mt-1 px-3 sm:px-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 bg-amber-50/30"><option value="member">${this.t.admin.roles.member}</option><option value="editor">${this.t.admin.roles.editor}</option><option value="treasurer">${this.t.admin.roles.treasurer}</option></select></div>
        <div class="flex gap-2 pt-2">
          <button class="modal-save-role flex-1 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 active:from-purple-700 active:to-indigo-700 text-white rounded-lg text-sm font-medium shadow-md shadow-purple-200">${this.t.admin.editRole}</button>
          <button class="modal-cancel px-4 py-2.5 bg-gray-100 active:bg-gray-200 text-gray-600 rounded-lg text-sm font-medium">${this.t.profile.cancel}</button>
        </div>
      </div>
    `;
