...
class AdminComponent {
      ...
      <div class="h-full overflow-y-auto bg-heritage-cream">
        <div class="p-4 sm:p-6 lg:p-8 border-b border-gray-200 bg-white">
          <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl sm:text-2xl font-bold font-serif text-gray-800">${this.t.admin.title}</h2>
          </div>
        </div>
        <div class="p-4 sm:p-6 lg:p-8">
          <div class="flex gap-3 mb-6 flex-wrap">
            ${['members', 'roles', 'gedcom'].map(tb => `<button class="admin-tab px-5 py-2.5 rounded-xl text-sm font-medium transition-all ${this.tab === tb ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md shadow-purple-200' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200'}" data-tab="${tb}">${tb === 'members' ? '👥 ' + this.t.admin.membersTab : tb === 'roles' ? '🏷️ ' + this.t.admin.rolesTab : '📄 ' + this.t.admin.gedcomTab}</button>`).join('')}
          </div>
  ...
  renderMembers(members) {
    return `<div class="space-y-3">${members.map(m => { const fullName = m.data['first name'] + ' ' + m.data['last name']; return `
      <div class="heritage-card p-4 lg:p-5 hover:shadow-md transition-all">
        <div class="flex items-center gap-4">
          ...
          <div class="flex-1 min-w-0">
            <button class="admin-member-link font-semibold text-gray-800 hover:text-purple-600 transition-colors" data-id="${m.id}">${fullName} ${m.data['death year'] ? '<span class="text-gray-400 text-sm">✝</span>' : ''}</button>
