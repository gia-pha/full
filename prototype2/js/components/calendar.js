...
class CalendarComponent {
  ...
  render() {
    ...
    this.container.innerHTML = `
      <div class="h-full overflow-y-auto bg-heritage-cream">
        <div class="p-4 sm:p-6 lg:p-8 border-b border-gray-200 flex flex-wrap items-center justify-between gap-3 bg-white">
          <div class="flex items-center gap-3">
            <div class="w-1 h-6 bg-gradient-to-b from-purple-600 to-indigo-600 rounded-full"></div>
            <h2 class="text-xl sm:text-2xl font-bold font-serif text-gray-800">${this.t.events.calendarView}</h2>
          </div>
        ...
        <div class="p-4 sm:p-6 lg:p-8">
          <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div class="flex items-center gap-2">
              <button class="cal-prev px-3 py-2 bg-white hover:bg-gray-50 border border-gray-200 rounded-lg text-gray-600 transition-colors">◀</button>
              <button class="cal-today px-3 py-2 bg-purple-100 hover:bg-purple-200 text-purple-700 rounded-lg text-sm font-medium transition-colors">${this.t.events.today}</button>
              <button class="cal-next px-3 py-2 bg-white hover:bg-gray-50 border border-gray-200 rounded-lg text-gray-600 transition-colors">▶</button>
            </div>
            <h3 class="text-lg font-bold font-serif text-gray-800">${monthNames[month]} ${year}${lunarYearName ? ' - ' + lunarYearName : ''}</h3>
          ...
          <div class="heritage-card overflow-hidden">
            <div class="grid grid-cols-7 bg-purple-50 border-b border-purple-100">
              ${dayNames.map(dn => `<div class="py-2 text-center text-xs font-semibold text-purple-600 uppercase">${dn}</div>`).join('')}
            </div>
  ...
  renderCell(cell, calendarType, lang) {
    if (!cell.current) {
      return `<div class="min-h-[80px] sm:min-h-[100px] p-1 border-b border-r border-gray-100 bg-gray-50/50">
        <span class="text-xs text-gray-300">${cell.day}</span>
      </div>`;
    }

    const todayClass = cell.isToday ? 'bg-purple-50 border-purple-300' : 'bg-white hover:bg-gray-50';
    const todayDayClass = cell.isToday ? 'bg-gradient-to-br from-purple-600 to-indigo-600 text-white rounded-full w-7 h-7 flex items-center justify-center' : '';
