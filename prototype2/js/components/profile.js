  ...
  render() {
    ...
    this.container.innerHTML = `
      <div class="h-full overflow-y-auto bg-heritage-cream">
        <div class="px-4 sm:px-6 lg:px-8 py-6 border-b border-gray-200 flex items-center justify-between bg-white">
          <div class="flex items-center gap-3">
            <div class="w-1 h-6 bg-gradient-to-b from-purple-600 to-indigo-600 rounded-full"></div>
            <h2 class="text-xl font-bold font-serif text-gray-800">${this.t.profile.title}</h2>
          </div>
          ${!this.editing ? `<button class="profile-edit-btn px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-lg text-sm font-medium transition-all shadow-md shadow-purple-200">✏️ ${this.t.profile.edit}</button>` : `<button class="profile-cancel px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg text-sm font-medium transition-colors mr-2">${this.t.profile.cancel}</button><button class="profile-save px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-lg text-sm font-medium transition-all shadow-md shadow-purple-200">${this.t.profile.save}</button>`}
        </div>
        <div class="px-4 sm:px-6 lg:px-8 py-6 pb-8">
          <div class="w-20 h-20 rounded-2xl ${person.data.gender === 'M' ? 'bg-gradient-to-br from-blue-100 to-indigo-100 text-blue-600' : 'bg-gradient-to-br from-pink-100 to-rose-100 text-pink-600'} shadow-lg flex items-center justify-center text-3xl border-2 border-white mb-4">${person.data.gender === 'M' ? '♂' : '♀'}</div>
          <div class="max-w-2xl space-y-6">
            <div>
              <label class="text-sm text-gray-400">${this.t.profile.name}</label>
              ${this.editing ? `<input type="text" class="profile-name w-full mt-2 px-5 py-3 border border-gray-300 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-500 bg-amber-50/30" value="${person.data['first name'] + ' ' + person.data['last name']}" />` : `<p class="mt-2 font-bold text-gray-800 text-xl">${person.data['first name'] + ' ' + person.data['last name']}</p>`}
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div><label class="text-sm text-gray-400">${this.t.profile.birthYear}</label>${this.editing ? `<input type="number" class="profile-birth w-full mt-2 px-5 py-3 border border-gray-300 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-500 bg-amber-50/30" value="${person.data.birthday || ''}" />` : `<p class="mt-2 text-gray-700">${person.data.birthday || '-'}</p>`}</div>
              <div><label class="text-sm text-gray-400">${this.t.profile.gender}</label>${this.editing ? `<select class="profile-gender w-full mt-2 px-5 py-3 border border-gray-300 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-500 bg-amber-50/30"><option value="M" ${person.data.gender === 'M' ? 'selected' : ''}>${this.t.member.male}</option><option value="F" ${person.data.gender === 'F' ? 'selected' : ''}>${this.t.member.female}</option></select>` : `<p class="mt-2 text-gray-700">${person.data.gender === 'M' ? this.t.member.male : this.t.member.female}</p>`}</div>
            </div>
            ...
            <div>
              ...
              <div class="space-y-3">${myClans.map(c => `<div class="heritage-card flex items-center gap-3 p-4"><span class="w-3 h-3 rounded-full ${c.lineage === 'father' ? 'bg-blue-500' : c.lineage === 'mother' ? 'bg-pink-500' : 'bg-purple-500'}"></span><span class="font-semibold text-gray-700">${c.name}</span><span class="text-sm text-gray-400 ml-auto">${this.t.clan.lineage[c.lineage]}</span></div>`).join('')}</div>
            </div>
